...
|---|---|
| Type | Numeric (float) |
| Range | 0.0 -- 1.0 (bounded by corpus min/max) |
| Unit | Mean Perspective API toxicity score |
| Example | `0.312` |
| Description | Mean Perspective API toxicity score across the regular posts in the participant's feed, excluding the fixed-position sponsored post. In this pilot, feeds were sampled using a Gaussian kernel centered on a target mean toxicity score, with targets drawn from a uniform grid spanning the corpus min/max and randomly assigned across participants. Note: this operationalization was replaced in the main study by a binary proportion measure (see main study codebook), because the continuous score distribution produced a bimodal rather than uniform distribution of realized feed toxicity across participants. |
